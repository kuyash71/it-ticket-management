...
export function Icon({ name, size = 16, style, className, fill = false, strokeWidth = 1.6 }) {
    const d = ICONS[name] ?? ICONS.info;
    const segments = d.split("M").filter(Boolean).map((seg) => "M" + seg);
    return (_jsx("svg", { className: className, width: size, height: size, viewBox: "0 0 24 24", fill: fill ? "currentColor" : "none", stroke: fill ? "none" : "currentColor", strokeWidth: strokeWidth, strokeLinecap: "round", strokeLinejoin: "round", style: style, "aria-hidden": "true", children: segments.map((seg, i) => _jsx("path", { d: seg }, i)) }));
}
